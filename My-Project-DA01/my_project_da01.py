# -*- coding: utf-8 -*-
"""My-project-DA01.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1oEXQlhzyH-Ct6YfODEs1Xo-oHDrLEc29
...
ev_city_distribution_top_counties = top_counties_data.groupby(['County', 'City']).size().sort_values(ascending=False).reset_index(name='Number of Vehicle')

#visualize the top 10 cities across these counties
top_cities = ev_city_distribution_top_counties.head(10)

plt.figure(figsize=(12, 8))
sns.barplot(x='Number of Vehicle', y='City', hue='County', data=top_cities, palette="magma")
...
#analyzing the ditribution of electric vehicle Types
...
plt.xlabel('Number of Vehicle Registeres')
...
plt.xlabel('Number of Vehicle Registered')
plt.ylabel('Electric Vehicle Manufacturer')
plt.tight_layout()
plt.show()
...
#filtering the dataser for these top manufacturers
top_makes_data = ev_data[ev_data['Make'].isin(top_3_makes)]

#analyzing the popularity of EV models within thesse top manufacturers
ev_model_distribution_top_makes = top_makes_data.groupby(['Make', 'Model']).size().sort_values(ascending=False).reset_index(name='Number of Vehicle')
...
plt.title('Top Models in Top Manufacturers by EV Registrations')
plt.xlabel('Number of Vehicle Registered')
plt.ylabel('Model')
...
# analyzing the ditribution of electric range
plt.figure(figsize=(10, 6))
sns.histplot(ev_data['Electric Range'], bins=30, kde=True, color='skyblue')
plt.title('Distribution of Electric vehicle Range')
plt.xlabel('Electric Range (miles)')
plt.ylabel('Number of Vehicle')
plt.axvline(ev_data['Electric Range'].mean(), color='red', linestyle='dashed', linewidth=2, label=f'Mean Range: {ev_data["Electric Range"].mean():.2f} miles')
plt.legend()
plt.show()
...
# calculating the average elctric range by mode year
...
#filter the dataser to include years with complete data, assuming 2023 is the last complete year
filtered_years = ev_registration_counts[ev_registration_counts.index <= 2023]

# define a function for expontential growth to fit the data
def exp_growth(x, a, b) :
  return a * np.exp(b * x)

#prepare the data for curve fitting
x_data = filtered_years.index.values - filtered_years.index.min()
y_data = filtered_years.values

#fit the data to the expontial growth function
params, covariance = curve_fit(exp_growth, x_data, y_data)

#use the fitted function to forecast the number of EVs for 224 and the next five years
forecast_years = np.arange(2024, 2024 + 6) - filtered_years.index.min()
forecast_values = exp_growth(forecast_years, *params)

# create a dictionary to display the forecasted values for easier interpertation
